...
export const load: PageServerLoad = async ({ parent }) => {
	...
	try {
		const data = await api.getWithAuth<{ categories: { id: number; name: string }[] }>('/admin/projects/project', token);
...
export const actions: Actions = {
	create: async ({ request, cookies }) => {
		...
		try {
			await api.postForm('/admin/projects/project', token, formData);
			return { success: true };
		} catch (err: unknown) {
