<script lang="ts">
	...
	const baseUrl = $derived(data.adminBaseUrl ?? '');
	...
	const pageTitle = $derived((data.projectsListMeta?.title as string) ?? (data.siteName ? `Projects | ${data.siteName}` : 'Projects'));
	const description = $derived((data.projectsListMeta?.description as string) ?? '');
</script>
