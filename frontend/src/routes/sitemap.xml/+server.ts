...
export const GET: RequestHandler = async () => {
	...
	const allUrlData = [
		...
		...(notDisabled(section.projects_enable)
			? [{ loc: `${BASE_URL}/projects`, changefreq: 'daily' as const, priority: 1.0, lastmod: new Date().toISOString() }]
			: []),
		...(notDisabled(section.articles_enable)
			? [{ loc: `${BASE_URL}/articles`, changefreq: 'daily' as const, priority: 1.0, lastmod: new Date().toISOString() }]
			: []),
		...aboutsUrlData,
		...articleUrlData,
		...projectUrlData
