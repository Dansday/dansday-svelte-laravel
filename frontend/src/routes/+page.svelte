<script lang="ts">
	...
	const general = $derived((data.general ?? {}) as Record<string, unknown>);
	const home = $derived((data.home ?? {}) as Record<string, unknown>);
	const metaTitle = $derived((general.title as string) ?? data.siteName ?? '');
	const metaDescription = $derived((general.description as string) ?? '');
</script>
